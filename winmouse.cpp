...
void winmouse::mouse_button_click(mouse_button_type_enum xmouse_enum)
{
  INPUT Input;
...
void winmouse::mouse_move(float Leap_Finger_x, float Leap_Finger_y)
{
  ...
  switch(mouse_type)
  {
    case xm_type_absolute:
        SetCursorPos(xy.x(), xy.y());
      break;
    case xm_type_relative:
      {
        if (prevPoint.x() == 0 && prevPoint.y() == 0)
          prevPoint = xy;
      ...
      break;
    case xm_type_absolute_to_relative:
      {
        POINT pt;
        GetCursorPos(&pt);
        Input.mi.dx = xy.x() - pt.x ;//x being coord in pixels;
        Input.mi.dy = xy.y() - pt.y;//y being coord in pixels;
        Input.mi.dwFlags = MOUSEEVENTF_MOVE;
        SendInput(1, &Input, sizeof(INPUT));
      }
      break;
    default:
      qWarning() << "winmouse::mouse_type not specified!";
      break;
  }
}

void winmouse::mouse_button_down(mouse_button_type_enum xmouse_enum)
{
  INPUT Input;
  Input.type = INPUT_MOUSE;
...
void winmouse::mouse_button_up(mouse_button_type_enum xmouse_enum)
{
  INPUT Input;
  Input.type = INPUT_MOUSE;
